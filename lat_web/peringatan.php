...
// Ambil data dari tabel peringatan
$result = $conn->query("SELECT * FROM peringatan");
...
$latestidperingatan = $stmt->fetch_assoc();
$urut = 1;
if ($latestidperingatan) {
    $latestNumber = $latestidperingatan['id_peringatan'];
    $urut = $latestNumber + 1;
}
$newidperingatan = $urut;
                            ...
                            <tbody>
                                ...
                                if ($result && $result->num_rows > 0) {
                                    ...
                                    while ($peringatan = $result->fetch_assoc()) {
                                        ...
                                        echo "<td class='text-center'>" . htmlspecialchars($peringatan['id_pegawai']) . "</td>";
                                        echo "<td class='text-center'>" . htmlspecialchars($peringatan['tanggal_peringatan']) . "</td>";
                                        echo "<td class='text-center'>" . htmlspecialchars($peringatan['alasan']) . "</td>";
                                        echo "<td class='text-center'>";
                                        echo "<div class='d-flex justify-content-center'>";
                                        echo "<button class='btn btn-warning btn-sm edit-btn mr-1' data-bs-toggle='modal' data-bs-target='#editdepartemenModal' data-id='" . htmlspecialchars($peringatan['id_peringatan']) . "'><i class='fas fa-edit'></i> Edit</button>";
                                        echo "<button class='btn btn-danger btn-sm delete-btn' data-id='" . htmlspecialchars($peringatan['id_peringatan']) . "'><i class='fas fa-trash'></i> Delete</button>";
                                        echo "</div>";
                                        echo "</td>";
                                        echo "</tr>";
                                    }
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="adddepartemenModalLabel">Add peringatan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="add_peringatan.php" method="post">
                    <div class="mb-3">
                    <label for="iddep" class="form-label">ID Peringatan</label>
                        <input type="text" class="form-control" id="iddep" name="iddep" value="<?php echo htmlspecialchars($newidperingatan); ?>" readonly>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label for="tanggal_peringatan" class="form-label">Tanggal Peringatan</label>
                        <input type="date" class="form-control" id="tanggal_peringatan" name="tanggal_peringatan" required>
                    </div>
                    <div class="mb-3">
